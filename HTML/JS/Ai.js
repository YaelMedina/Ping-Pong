...
//Bot del juego 
const reactionSpeed = .3;
const moveBot = () => {
    const maxSpeed = 3; // Ajusta según sea necesario
    if (ball.isMoving && ball.directionX === 'right' && ball.x > canvas.width / 2) {
        const middleOfPaddle = paddleRight.y + paddleRight.h / 2;
        const middleOfBall = ball.y + ball.h / 2;
        sleep (350)//Retrasar el timepo de respuesta del bot
        ...
        // Limitar la velocidad máxima del bot
        if (Math.abs(paddleRight.speed) > maxSpeed) {
            paddleRight.speed = reactionSpeed * maxSpeed * Math.sign(paddleRight.speed);
        }
    }
};
...
const getPaddle = ({x = 5, color = '#2980B9'}) => ({
    ...
    x: x,
    y: 5, //posicion de la paleta izq. = 225 (centro)
    w: 15,
    h: 100,
    color: color,
    speed: 20,
...
const getBall = () => ({
    x: canvas.width / 2, //Posicion en X de la pelota
    y: canvas.height / 2,//Posicion en Y de la pelota
    w: 25,
    h: 25,
    color: 'silver',
    directionX: 'right',
